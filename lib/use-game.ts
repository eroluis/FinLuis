...
function costeInversionConMod(inv: Inversion, mod: Modificadores): number {
  return Math.max(0, inv.coste + mod.inversionDelta)
}

function costeCompraConMod(compra: Compra, mod: Modificadores): number {
...
function pedirCreditoJugador(j: Jugador): { jugador: Jugador; pagaresNuevos: number } {
  if (j.pagares >= MAX_PAGARES) return { jugador: j, pagaresNuevos: 0 }
  const enCredito = j.zona === "credito"
  const barato = enCredito && !j.creditoBaratoUsado
  const base = barato ? 5 : 6
  const pagaresNuevos = Math.min(base, MAX_PAGARES - j.pagares)
  return {
    jugador: {
      ...j,
      efectivo: j.efectivo + 600,
      pagares: j.pagares + pagaresNuevos,
...
function cubrirGasto(j: Jugador, monto: number, mensajes: string[]): Jugador {
  ...
  if (jug.ahorro > 0 && faltante > 0) {
    const retiro = Math.min(jug.ahorro, Math.ceil(faltante / 0.9))
...
function estadoInicialVacio(): EstadoJuego {
  return {
    fase: "config",
    anio: 1,
    zonaActual: 1,
